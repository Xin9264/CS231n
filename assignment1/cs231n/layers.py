...
def affine_forward(x, w, b):
    ...
    X = x.reshape(x.shape[0], -1)  # reshape x.shape -> (N, D)
    out = np.dot(X, w) + b
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        sample_variance = sample_mean^2 - np.mean(np.square(x), axis=1)
        sample_variance = np.sqrt(sample_variance)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
...
def svm_loss(x, y):
    ...
    real_scores = x[range(len(y)), y].reshape(-1, 1)
    margin = np.maximum(0, x - real_scores + 1)
    margin[np.arange(len(y)), y] = 0
    loss = np.sum(margin) / x.shape[0]
    dx = np.zeros_like(x)
    dx[margin > 0] = 1
    dx[range(len(y)), y] -= np.sum(dx, axis=1)
    dx = dx / x.shape[0]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
