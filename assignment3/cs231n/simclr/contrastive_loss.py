...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    def denominator(N, i, z_i, z_k, tau):
        result = 0
        for k in range(2*N):
            if k == i:
                result = result 
            else:
                result = result + torch.exp(sim(z_i, z_k[k]) / tau)
        return result
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss = total_loss - torch.log(torch.exp(sim(z_k, z_k_N) / tau) / denominator(N, k, z_k, out, tau)) 
        total_loss = total_loss - torch.log(torch.exp(sim(z_k_N, z_k) / tau) / denominator(N, k, z_k_N, out, tau)) 

        # sim_k = torch.tensor([sim(z_k, z) for z in out[np.arange(2*N)!=k]])
        # sim_k_N = torch.tensor([sim(z_k_N, z) for z in out[np.arange(2*N)!=k]])

        # l1 = torch.exp(sim(z_k, z_k_N) / tau) / torch.exp(sim_k / tau).sum()
        # l2 = torch.exp(sim(z_k_N, z_k) / tau) / torch.exp(sim_k_N / tau).sum()
...
def sim_positive_pairs(out_left, out_right):
    ...
    # pos_pairs = torch.matmul(out_left, out_right.T) / (torch.linalg.norm(out_left, dim=1, keepdim=True) * torch.linalg.norm(out_right, dim=1, keepdim=True))
