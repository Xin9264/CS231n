...
def compute_saliency_maps(X, y, model):
    ...
    loss.backward()
    # gradients of the input images
    gradients = X.grad
    # get the maxium gradient
    saliency = torch.abs(gradients).max(dim=1)[0]
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(max_iteration):
        ...
        if torch.argmax(scores) == target_y:
            print(f"Successfully fool the model after {i+1} iterations.")
            break
        target_score = scores[:, target_y]
        target_score.backward()
        with torch.no_grad():
            # Calculate the L2 norm of the gradient
            gradient_norm = torch.norm(X_fooling.grad)
            # Update the fooling image using gradient ascent
            X_fooling.data += (learning_rate * X_fooling.grad / gradient_norm)
