...
def rnn_step_backward(dnext_h, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    '''
    In this function, I made a big mistake. For this reason, I spend a lot of time find the bug.
    I am so stupid, always.
    '''

    prev_h, Wx, Wh, x, a = cache
    
    # def dtanh(a):
    #     return 1 - np.tanh(a)**2
    next_h = np.tanh(a)
    da = (1 - np.square(next_h)) * dnext_h
    # print(f"shape of da = {da.shape}, shape of Wx.T = {Wx.T.shape}")
    db = np.sum(da, axis=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # cache = np.zeros((T, 5))
    h = np.zeros((T, N, H))
    h_current, cache0 = rnn_step_forward(x_t[0], h0, Wx, Wh, b)
    prev_h, _, _, _, a = cache0
    cache_list = []
    cache_list.append((prev_h, Wx, Wh, x_t[0],  a))
    h[0] = h_current
    for i in range(1, T):
        h[i], temp_cache = rnn_step_forward(x_t[i], h[i-1], Wx, Wh, b)
        prev_h, _, _, _, a = temp_cache
        cache_list.append((prev_h, Wx, Wh, x_t[i], a))
    h = h.transpose((1, 0, 2))
    cache = cache_list
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    dWh = np.zeros_like(cache[0][2])
    db = np.zeros_like(cache[0][4])
    dx = np.zeros((T, N, D))
    dprev_h = np.zeros((N, H))

    for i in reversed(range(T)):
        dx_temp, dprev_h, dWx_temp, dWh_temp, db_temp = rnn_step_backward(dh_t[i] + dprev_h, cache[i])
        dx[i] = dx_temp
        dWx += dWx_temp
        dWh += dWh_temp
        db += db_temp
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    N, D = x.shape
    N, H = prev_c.shape
    a = x @ Wx + prev_h @ Wh + b.reshape(1, 4*H)
    # print(a.shape, H)
    a_i = a[:, :H]
    a_f = a[:, H:2*H]
    a_o = a[:, 2*H:3*H]
    ...
    cache = (prev_h, prev_c, next_c, Wx, Wh, i, f, o, g, x)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    (prev_h, prev_c, next_c, Wx, Wh, i, f, o, g, x) = cache
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape
    N, H = h0.shape
    x_t = np.moveaxis(x, 1, 0)
    c = np.zeros((N, H))
    cache = []
    h = np.zeros((T, N, H))
    for t in range(T):
        h0, c, cache_temp = lstm_step_forward(x_t[t], h0, c, Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    dh_t = np.moveaxis(dh, 1, 0)
    prev_h, prev_c, next_c, Wx, Wh, i, f, o, g, x = cache[T-1]
    N, D = x.shape
    H4 = 4 * H
    dx = np.empty((N, T, D))
    dh0 = np.zeros((N, H))
    ...
    for t in range(T-1, -1, -1):
        dx_, dh0, dprev_c, dWx_, dWh_, db_ = lstm_step_backward(dh0+dh[:, t], dprev_c, cache[t])
        dx[:, t] = dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
